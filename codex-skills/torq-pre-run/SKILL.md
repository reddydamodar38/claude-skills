...
## Domain Inventory Sources

When a domain's CTX/SUT list is missing or looks incomplete, check the source inventory before asking the user for every node.

Primary sources:

- Node orchestration host: `dh2vrbbablcert1`
- Node orchestration path: `/root/node-orchestration/`
- Source repo: `https://github.cerner.com/abilities-center/node-orchestration`

Credential handling:

- Do not hardcode passwords in the skill or generated commands.
- Use an existing approved shell/session or prompt securely for credentials when connecting to `dh2vrbbablcert1`.

TORQ/Jenkins fallback:

- If not all nodes are present in node-orchestration, check the TORQ/Jenkins domain page.
- Default pattern: `http://dh2torqvip1.dh2.cerner.com/<domain>/`
- Scale examples:
  - `http://dh2torqvip1.dh2.cerner.com/ablscale1/`
  - `http://dh2torqvip1.dh2.cerner.com/ablscale2/`
  - `http://dh2torqvip1.dh2.cerner.com/ablscale3/`
  - `http://dh2torqvip1.dh2.cerner.com/ablscale4/`

Known TORQ domain slugs from the ABL index:

- Ablcert: `cldua`, `abla`, `ablb`, `ablc`, `abld`, `able`, `ablf`, `ablg`, `ablh`, `abli`, `ablj`, `ablk`, `abll`, `ablm`, `abls`, `csintop`, `ablscale1`, `ablscale2`, `ablscale3`, `ablprep1`, `ablprep2`
- MillRev/CAMM: `ablr`, `clmstrat`, `camm7a`, `mrevc`, `mrevd`, `mreve`
- CaU: `bench`, `lin64`, `flaws`, `fltnx`, `flelk`, `crest`; `lntec` uses `http://dh2vcautorq01:8087/lntec/`
- Development: `abltls`, `tools`, `abldev`
